...
function AnimatedTorch({ mirror = false }: { mirror?: boolean }) {
  return (
    <div
      className={`relative w-16 flex-shrink-0 self-center ${mirror ? '-scale-x-100' : ''}`}
      aria-hidden
    >
      <Image
        src="/torch.png"
      ...
      <div className="absolute inset-0 flex justify-center top-0">
        <div className="relative w-10 mt-0.5">
          {/* Radial orange glow centered behind the fire */}
          <div
            className="absolute left-1/2 w-[5rem] h-[5rem] -translate-x-1/2 -translate-y-1/2 rounded-full pointer-events-none"
            style={{
              top: 'calc(50% - 12px)',
...
export function SignInForm({ callbackUrl = '/' }: SignInFormProps) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-coliseum-black p-6">
      {/* Top row: two torches flanking the card */}
      <div className="flex items-center gap-6 sm:gap-8">
        ...
        <div className="w-full max-w-md space-y-8 rounded-sm border-2 border-coliseum-bronze/30 bg-coliseum-stone p-8 shadow-xl shadow-black/50">
          <div className="text-center space-y-2">
            <h1 className="font-display text-4xl tracking-wide text-coliseum-sand uppercase">
              Gladiator Coliseum
            </h1>
      ...
      {/* Bottom row: two torches closer to box, farther out horizontally */}
