...
| **6** | Multiplayer — Real-time PvP (matchmaking queue, friends, challenges, 60Hz sim/20Hz broadcast, Redis scaling, input validation, disconnect handling) | ✅ Complete |
| **7** | Polish, testing & deployment (bug fixes, tests, Vercel + Railway, mainnet guide) | 🔄 In Progress |

**Currently built:** Monorepo (pnpm, Turborepo), Next.js 14 frontend (auth, wallet, mint, **admin UI** — game data authoring with **structured equipment UI metadata** editing, **admin/users** Manage Users with **create test Gladiator** (class selection, confirmation), **admin announcements**, **admin activity logs**; **arena** at `/arena` with active gladiator selection; **camp** at `/camp` for managing gladiators, inventory, crafting, and spending stat/skill points; **match page** with Canvas, `MatchHUD`, `WeaponSelector`, client prediction, 4 weapons, projectiles; **match history** at `/matches`; **progression** (level, XP bar, **6 cross-class skill trees** with **SkillTreeContext** single-fetch caching and performance polish, stat points with 3 per level); **loot boxes** (inventory, open, reward modal); **equipment** (inventory with **icons from API-enriched data**, equip, **crafting** 3→1, **salvage** for gold); **quick match** at `/quick-match` (real gladiator selection, joins matchmaking queue); **friends & challenges** at `/friends` (full friend/request/challenge management with real data); **notifications** (persistent HUD bell, dropdown, real-time count, friend requests, arena challenges, admin logs, public announcements); marketing/landing, Blood & Bronze UI), Express game server (real-time combat engine (60Hz sim / 20Hz broadcast), multi-weapon, projectiles, CPU AI; **match persistence**, **rewards**, **XP/progression**, **loot drops**, **PvP match creation via matchmaking/challenges**; **input validation & rate limiting**, **disconnect/reconnect handling**; Redis-backed Socket.io adapter for scaling; bundle loader, blockchain listener), **packages/shared** (physics, combat, **loot** starter-gear, **skills** skill-trees, **crafting** 3→1), Gladiator NFT (Hardhat, 8 stats), Supabase + Prisma (**Match** completion/rewards, **LootBox**, **UserGold**, **Friend**/Challenge relations, **Notification**/Announcement).
...
| [docs/plans/summaries/EQUIPMENT-UI-METADATA-IMPLEMENTATION.md](docs/plans/summaries/EQUIPMENT-UI-METADATA-IMPLEMENTATION.md) | Equipment UI metadata implementation (DB/bundles as source of truth; Admin UI structured form; /api/equipment enrichment; inventory icons) |
...
| [docs/features/admin-ui.md](docs/features/admin-ui.md) | Admin UI plan — game data authoring, CRUD, validation, publish/export, Manage Users (create test Gladiator) |
| [docs/features/skill-trees-cross-class.md](docs/features/skill-trees-cross-class.md) | Cross-class skill trees — player-facing overview (6 trees, builds, prerequisites) |
| [docs/features/skill-trees-system.md](docs/features/skill-trees-system.md) | Skill tree system — how it works, storage, fetching, performance (technical reference) |
